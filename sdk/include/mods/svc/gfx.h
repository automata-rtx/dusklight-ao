...
/*
 * The scene's surface normals, snapshotted once per frame after the opaque lists while at least one
 * mod is registered for them. view is NULL earlier in the frame than the snapshot.
 */
...
typedef struct GfxService {
    ...
    /*
     * MOD_UNSUPPORTED means the renderer cannot produce scene normals, as on the compatibility
     * renderers. The snapshot runs while at least one mod is registered, so a mod that toggles its
     * effect on and off does not pay for the capture starting again; the registration is dropped
     * when the mod is deactivated.
     */
    ModResult (*register_scene_normals)(ModContext* ctx);
    ModResult (*unregister_scene_normals)(ModContext* ctx);
