...
void on_scene_after_opaque(ModContext*, const GfxStageContext* stageCtx, void*) {
    ...
    if (normals.view == nullptr) {
        if (!g_warnedNoNormals) {
            g_warnedNoNormals = true;
            svc_log->warn(mod_ctx, "scene normals unavailable; AO disabled");
        }
        return;
    }
