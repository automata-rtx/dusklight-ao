...
13. **Dense weather particles cost far more than they should.** Reported
    2026-08-07: rain in Hyrule Field and snow in the Snowpeak exteriors and
    Snowpeak Ruins run at unusable frame rates. Two owner observations frame
    the whole entry, and both turned out to be diagnostic:
    ...
    **Regression signature:** rain or snow vanishing, drawing in a single flat
    ...
    is unbalanced). **Syntax-checked with the MinGW harness, untested in game.**

    **Not done, and deliberately:** `dKyr_mud_draw` and `dKyr_evil_draw2` still
    emit per-quad and still set `GX_TEVREG0` per particle, so they need the same
    TEV→vertex-colour rework rain got. They are bounded (mud is 100 effects) and
    situational rather than weather, so they were left rather than changed
    blind alongside a change that cannot be tested here.
