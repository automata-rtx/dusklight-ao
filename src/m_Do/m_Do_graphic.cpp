    ...
    if (dComIfGp_getWindowNum() != 0) {
        ...
        if (camera_p != NULL) {
            ...
            if (dusk::getSettings().game.shadowDebugMode.getValue() != 0) {
                // Anchor the shadow frustum on the player; fall back to the camera target if absent.
                fopAc_ac_c* player_p = dComIfGp_getPlayer(0);
                ...
                if (!s_haveSun || (dt < 0.0f ? -dt : dt) >= kSunSnapStep) {
                    ...
                    if (len > 1.0f && dir.y / len < 0.8f) {
                        ...
                        if (horiz2 > 0.0001f) {
                            const f32 sc = sqrtf((len * len - dir.y * dir.y) / horiz2);
                            dir.x *= sc;
                            dir.z *= sc;
                        }
                    }
...
#if TARGET_PC
            // Exclude projected/blob shadows (incl. Link's) from AO: they are flat dark
            // decals the renderer can't tell from solid world geometry, and they pollute
            // the AO normal/mask (square artifacts under rocks, Link's shadow darkening).
            aurora_ao_set_world_exclude(true);
#endif
            GX_DEBUG_GROUP(dComIfGd_drawShadow, camera_p->view.viewMtx);
#if TARGET_PC
            aurora_ao_set_world_exclude(false);
#endif
