...
std::string build_shader_source(const ShaderConfig& config) noexcept {
  ...
  if constexpr (EnableAOData) {
    vtxOutAttrs += fmt::format("\n    @location({}) ao_nrm: vec3f,", vtxOutIdx++);
    vtxXfrAttrsPre += "\n    out.ao_nrm = mv_nrm;";
  }
  ...
  // Second render target for ambient occlusion: view-space normal (rgb) plus an
  // "ambient-lit" mask (a). The mask is 1 for surfaces shaded by GX lighting and
  // 0 for unlit/emissive/additive materials, so AO never darkens glow or UI.
  std::string aoOutputStruct;
  std::string aoReturnType;
  std::string aoReturnStmt;
  if constexpr (EnableAOData) {
    // The fragment always writes mask = 1 (this surface exists); whether it
    // counts as a solid AO-eligible surface is decided by the pipeline's blend
    // state for the AO-data target (solid draws keep 1, translucent/additive
    // effect draws force it to 0). This works regardless of whether the surface
    // uses GX hardware lighting (TP bakes most lighting into vertex colors).
    aoOutputStruct =
        "struct FragmentOutput {\n"
        "    @location(0) color: vec4f,\n"
        "    @location(1) ao_data: vec4f,\n"
        "};\n";
    aoReturnType = "FragmentOutput";
    aoReturnStmt =
        "\n    var fout: FragmentOutput;"
        "\n    fout.color = prev;"
        "\n    let ao_n = select(vec3f(0.0, 0.0, 1.0), normalize(in.ao_nrm), dot(in.ao_nrm, in.ao_nrm) > 1e-8);"
        "\n    fout.ao_data = vec4f(ao_n * 0.5 + 0.5, ubuf.ao_mask);"
        "\n    return fout;";
  } else {
    aoReturnType = "@location(0) vec4f";
    aoReturnStmt = "\n    return prev;";
  }
...
fn fs_main(in: VertexOutput) -> {10} {{{6}{5}{11}
                                        ...
                                        fragmentFnPre, vtxXfrAttrsPre, uniformPre, aoOutputStruct, aoReturnType,
                                        aoReturnStmt);
