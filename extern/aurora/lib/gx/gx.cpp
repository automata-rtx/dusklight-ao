...
wgpu::RenderPipeline build_pipeline(const PipelineConfig& config, ArrayRef<wgpu::VertexBufferLayout> vtxBuffers,
  ...
  // The AO-data target carries the per-draw "AO-eligible" mask in alpha (set in
  // the uniform: 1 for the solid perspective world, 0 otherwise). The alpha blend
  // is out.a = src.a + dst.a*(1-src.a): a draw with mask 1 stamps 1, but a draw
  // with mask 0 PRESERVES whatever is already there. This is essential because
  // TP draws many full-screen orthographic EFB effects (color washes, bloom
  // composite) over the world after it; without preserve they would wipe the
  // world's mask back to 0 and AO would vanish everywhere except isolated 3D
  // renders (e.g. the equipment model window).
  static constexpr wgpu::BlendState aoDataPreserve{
  ...
  // Additive/subtractive draws are glow/particle/fog effects. Force their mask to
  // 0 (and leave the normal alone) so AO never lands on them, even over the world.
  static constexpr wgpu::BlendState aoDataEffect{
      .color = wgpu::BlendComponent{.operation = wgpu::BlendOperation::Add,
                                    .srcFactor = wgpu::BlendFactor::One,
                                    .dstFactor = wgpu::BlendFactor::Zero},
      .alpha = wgpu::BlendComponent{.operation = wgpu::BlendOperation::Add,
  ...
  const bool aoEffect = (config.blendMode == GX_BM_BLEND && config.blendFacDst == GX_BL_ONE) ||
                        config.blendMode == GX_BM_SUBTRACT;
  const std::array colorTargets{
      ...
      wgpu::ColorTargetState{
          ...
          .blend = aoEffect ? &aoDataEffect : &aoDataPreserve,
          .writeMask = aoEffect ? wgpu::ColorWriteMask::Alpha
                                : (config.colorUpdate ? wgpu::ColorWriteMask::All : wgpu::ColorWriteMask::None),
      },
  };
  const wgpu::FragmentState fragmentState{
      .module = shader,
      .entryPoint = "fs_main",
      .targetCount = EnableAOData ? 2u : 1u,
