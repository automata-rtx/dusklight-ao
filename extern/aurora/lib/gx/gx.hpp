...
// When set, the GX 3D pass writes a second render target (view-space normal +
// an "ambient-lit" mask) that the ambient-occlusion pass consumes so it only
// darkens lit surfaces (never glow/unlit/additive/UI).
constexpr bool EnableAOData = true; // (briefly toggled false to test the stutter; MRT was not the cause)
