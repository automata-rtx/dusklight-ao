...
static void handle_bp(u32 value, bool bigEndian) {
  ...
  case 0xF1: {
    ...
    // Record the scene fog for fog-aware AO. The A/B params (0xEE-0xF0) are written
    // before the type/C (0xF1), so g_gxState.fog is complete here.
    gfx::ao::note_scene_fog(static_cast<int>(fogType), g_gxState.fog.a, g_gxState.fog.b, g_gxState.fog.c);
    g_gxState.stateDirty = true;
    break;
  }
      ...
      case 0x26: {
        ...
        if (projOff == 0 && count >= 7) {
          ...
          if (projType == GX_ORTHOGRAPHIC) {
            ...
            proj.m1[3] = p3;
            proj.m3[3] = 1.0f;
            // First perspective->ortho transition of the frame: apply AO to the
            // EFB now, before the HUD / UI / ortho post-effects draw over it.
            gfx::ao_apply_positional();
          } else {
            proj.m0[2] = p1;
            proj.m1[2] = p3;
            proj.m3[2] = -1.0f;
            // Record the scene perspective projection for the AO pass, and latch
            // that 3D geometry is being drawn this frame.
            gfx::ao::set_scene_projection(proj);
            gfx::ao_mark_perspective();
          }
          g_gxState.stateDirty = true;
        }
        break;
