...
gfx::Range build_uniform(const ShaderInfo& info, u32 vtxStart, const BindGroupRanges& ranges) noexcept {
  ...
  // ao_mask: 1 for solid 3D scene geometry that AO should darken, 0 otherwise.
  // Perspective draws are the 3D world; orthographic draws are UI / HUD /
  // letterbox bars. Additive/subtractive draws are glow/particle effects.
  const bool aoAdditive = (g_gxState.blendMode == GX_BM_BLEND && g_gxState.blendFacDst == GX_BL_ONE) ||
                          g_gxState.blendMode == GX_BM_SUBTRACT;
  const f32 aoMask = (g_gxState.projType == GX_PERSPECTIVE && !aoAdditive) ? 1.0f : 0.0f;
  buf.append<f32>(aoMask);
  buf.append_zeroes(4); // pad
