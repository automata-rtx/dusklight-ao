...
#ifdef AURORA_ENABLE_GX
  auto opts = aurora::gfx::ao::options();
  opts.enabled = enabled;
  aurora::gfx::ao::set_options(opts);
#else
  (void)enabled;
#endif
}
...
#ifdef AURORA_ENABLE_GX
  auto opts = aurora::gfx::ao::options();
  opts.resolutionDivisor = divisor;
  aurora::gfx::ao::set_options(opts);
#else
  (void)divisor;
#endif
...
#ifdef AURORA_ENABLE_GX
  auto opts = aurora::gfx::ao::options();
  opts.fogFadeStrength = strength;
  opts.fogFadeStart = start;
  aurora::gfx::ao::set_options(opts);
#else
  (void)strength;
  (void)start;
...
void aurora_apply_ao_now() {
#ifdef AURORA_ENABLE_GX
  // Cheap early-out when disabled so the AO-off path skips the FIFO drain.
  if (!aurora::gfx::ao::options().enabled) {
    return;
  }
  // Flush all GX commands issued so far (the opaque scene) into the recording,
  // then insert the AO apply before any subsequent (translucent/particle) draws.
  aurora::gx::fifo::drain();
  aurora::gfx::ao_apply_positional();
#endif
}
