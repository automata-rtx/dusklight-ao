...
// Positional screen-space AO. The command processor latches that 3D (perspective)
// geometry was drawn this frame (ao_mark_perspective), then on the first
// perspective->orthographic transition applies AO to the EFB in place
// (ao_apply_positional) so that HUD / UI / ortho post-effects draw on top of the
// AO'd world rather than being darkened by it. Falls back to an end_frame apply
// on frames with no such transition.
void ao_mark_perspective() noexcept;
