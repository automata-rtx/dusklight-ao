...
wgpu::RenderPipeline create_pipeline(const PipelineConfig& config) {
  ...
  sourceDescriptor.code = gx::EnableAOData ? R"""(
struct VertexOutput {
...
struct FragmentOutput {
    ...
    @location(1) ao_data: vec4<f32>,
};

@fragment
fn fs_main() -> FragmentOutput {
    var o: FragmentOutput;
    o.color = vec4<f32>(1.0);
    o.ao_data = vec4<f32>(0.0);
    return o;
}
)""" : R"""(
struct VertexOutput {
...
fn fs_main() -> @location(0) vec4<f32> {
  ...
  const std::array colorTargets{
      ...
      // AO-data target: left untouched by clears (mask written by real geometry).
      wgpu::ColorTargetState{
  ...
  const wgpu::FragmentState fragmentState{
      .module = module,
      .entryPoint = "fs_main",
      .targetCount = gx::EnableAOData ? 2u : 1u,
