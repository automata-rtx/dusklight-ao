...
struct RenderPass {
  ...
  wgpu::TextureView aoDataView; // second color target (AO normal + lit mask); null = none
...
// Positional AO latches, reset each frame in begin_frame.
static bool g_aoSawPerspective = false; // a perspective projection was set this frame
static bool g_aoApplied = false;        // AO was applied positionally this frame
...
static webgpu::TextureWithSampler g_offscreenAoData;
...
static void set_efb_targets(RenderPass& pass) {
  ...
  if constexpr (gx::EnableAOData) {
    pass.aoDataView = webgpu::g_aoDataBuffer.view;
  }
...
struct OffscreenCacheEntry {
  ...
  webgpu::TextureWithSampler aoData;
};
...
void resolve_pass(TextureHandle texture, ClipRect rect, bool clearColor, bool clearAlpha, bool clearDepth,
  ...
  RenderPass newPass{
      ...
      .aoDataView = prevPass.aoDataView,
...
static OffscreenCacheEntry get_offscreen_textures(uint32_t width, uint32_t height) {
  ...
  if constexpr (gx::EnableAOData) {
    // GX pipelines always declare the AO-data target, so offscreen passes need a
    // matching attachment even though the AO pass never reads it.
    const wgpu::TextureDescriptor aoDesc{
        .label = "Offscreen AO Data",
        .usage = wgpu::TextureUsage::RenderAttachment,
        .dimension = wgpu::TextureDimension::e2D,
        .size = size,
    ...
    auto aoTexture = g_device.CreateTexture(&aoDesc);
    auto aoView = aoTexture.CreateView();
    entry.aoData = webgpu::TextureWithSampler{
        .texture = std::move(aoTexture),
        .view = std::move(aoView),
        .size = size,
        .format = wgpu::TextureFormat::RGBA8Unorm,
    };
  }
...
void begin_offscreen(uint32_t width, uint32_t height) {
  ...
  g_offscreenAoData = std::move(offscreenEntry.aoData);

  // Start a new pass with offscreen targets
  RenderPass newPass{
      .colorView = g_offscreenColor.view,
      .aoDataView = gx::EnableAOData ? g_offscreenAoData.view : wgpu::TextureView{},
...
void ao_mark_perspective() noexcept { g_aoSawPerspective = true; }

void ao_apply_positional() noexcept {
  ZoneScoped;
  // No mask buffer without the AO-data MRT: nothing to apply, and splitting the
  // pass would only add cost. Compiled out entirely when EnableAOData is on.
  if constexpr (!gx::EnableAOData) {
    return;
  }
  // Only the first perspective->ortho transition of a frame, while actively
  // recording the EFB pass (not offscreen / between passes).
  if (g_recordingFrame == nullptr || g_inOffscreen || g_currentRenderPass == UINT32_MAX) {
    return;
  }
  if (g_aoApplied || !g_aoSawPerspective) {
    return;
  }
  // Cheap early-out when AO is disabled, before the capture_frame_params matrix
  // inverse below. The ortho trigger calls this on every orthographic projection,
  // so keep the AO-off path near-zero cost.
  if (!ao::options().enabled) {
    return;
  }
  // AO can't run on multisampled depth; don't split the pass for nothing.
  if (webgpu::g_graphicsConfig.msaaSamples > 1) {
    return;
  }
  // Snapshot AO params on the main thread (the worker closure must not read live
  // GX state). If AO is disabled or the params are invalid, leave the pass intact
  // so the AO-off path is byte-identical to the no-split behaviour.
  const ao::FrameParams params = ao::capture_frame_params();
  if (!params.valid || !params.enabled) {
    return;
  }
  // Debug views apply at finish() (over the final EFB) instead of here, so the
  // visualization isn't overdrawn by the post-hook translucent/HUD/post-fx passes.
  ...
  // Seal & enqueue the world EFB pass so the worker encodes it before AO runs.
  // This split is not driven by a resolve target, so the pass must be marked
  // observable or render() would skip it (and the world would never be drawn).
  current_render_passes()[g_currentRenderPass].observable = true;
  enqueue_pass(frame, frameSlot, g_currentRenderPass);

  // Inject the AO apply directly into the worker's FIFO op stream, right after
  // the world pass and before any subsequent (ortho) passes. Mirrors enqueue_op's
  // closure but runs ao::render in place of encoding a recorded pass.
  render_worker::enqueue_encode_pass(frame.frameId, g_currentRenderPass, [frameSlot, params] {
    auto& packet = g_framePackets[frameSlot];
    ao::render(packet.encoder, webgpu::g_frameBuffer, webgpu::g_depthBuffer, params);
  });

  // Resume EFB rendering in a fresh pass that LOADS the now-AO'd content, so the
  // HUD / UI / ortho post-effects draw on top of AO instead of being darkened.
  auto& pass = current_render_passes().emplace_back();
  pass.clearColor = false;
  pass.clearDepth = false;
  ++g_currentRenderPass;
...
void finish() {
  ...
  if (g_currentRenderPass != UINT32_MAX) {
    ...
    enqueue_pass(frame, g_recordingFrameSlot, g_currentRenderPass);
    // Debug AO views: apply over the final EFB here (after every world/translucent/
    // HUD/post-fx draw, before the RmlUi composite) so the visualization is neither
    // overdrawn by later passes nor bypassed by the RmlUi present path. Normal AO
    // applies positionally at the perspective->ortho hook instead.
    if constexpr (gx::EnableAOData) {
      if (!g_aoApplied && webgpu::g_graphicsConfig.msaaSamples == 1) {
        const ao::FrameParams params = ao::capture_frame_params();
        if (params.valid && params.enabled && params.debugMode != 0) {
          g_aoApplied = true;
          const size_t frameSlot = g_recordingFrameSlot;
          render_worker::enqueue_encode_pass(frame.frameId, g_currentRenderPass, [frameSlot, params] {
            auto& packet = g_framePackets[frameSlot];
...
void end_frame(EndFrameCallback callback) {
  ...
  // Snapshot the scene projection on the main thread; the worker closure below
  // must not read live GX state.
  const ao::FrameParams aoParams = ao::capture_frame_params();
  // If AO was already applied positionally (before the ortho UI), don't apply it
  // again here. This end_frame apply only covers frames with no perspective->ortho
  // transition (e.g. pure-3D frames with no HUD/UI).
  const bool aoAlreadyApplied = g_aoApplied;
  render_worker::enqueue_end_frame(
      frameId, [frameSlot, stagingSlot, aoParams, aoAlreadyApplied, callback = std::move(callback)]() mutable {
    auto& packet = g_framePackets[frameSlot];
    ...
    // Screen-space AO fallback: modulate the high-resolution EFB color before the
    // present resample downsamples it. No-op when disabled, msaa > 1, or when AO
    // was already applied positionally this frame.
    if (!aoAlreadyApplied) {
      ao::render(encoder, webgpu::g_frameBuffer, webgpu::g_depthBuffer, aoParams);
    }
    if (callback) {
      callback(encoder);
    }
...
static void render(wgpu::CommandEncoder& cmd, FramePacket& frame, RenderPass& passInfo, uint32_t passIndex) {
  ...
  const auto colorLoadOp = passInfo.colorLoadOp != wgpu::LoadOp::Undefined
  ...
  if (passInfo.aoDataView) {
    attachments[1] = wgpu::RenderPassColorAttachment{
        .view = passInfo.aoDataView,
        .loadOp = colorLoadOp,
