...
TextureWithSampler g_aoDataBuffer;
...
static TextureWithSampler create_ao_data_texture(uint32_t width, uint32_t height) {
  const wgpu::Extent3D size{.width = width, .height = height, .depthOrArrayLayers = 1};
  const wgpu::TextureDescriptor textureDescriptor{
      .label = "AO Data texture",
...
void resize_swapchain(uint32_t width, uint32_t height, uint32_t native_width, uint32_t native_height, bool force) {
  ...
  if constexpr (gx::EnableAOData) {
    g_aoDataBuffer = create_ao_data_texture(width, height);
  }
